...
public:
    ...
    SphereStereoNode() : Node("sphere_stereo_node")
    {
        ...
        this->declare_parameter("publish_fps", 10.0);
        ...
        matching_width_ = this->get_parameter("matching_width").as_int();
        matching_height_ = this->get_parameter("matching_height").as_int();
        ...
        RCLCPP_INFO(this->get_logger(), "Matching resolution: %dx%d", matching_width_, matching_height_);
        
        try {
            // Initialize calibration
            RCLCPP_INFO(this->get_logger(), "Loading calibration...");
            Vec2f matching_resolution(matching_width_, matching_height_);
            calibration_ = std::make_shared<CalibrationSet>(calibration_file, matching_resolution);
            RCLCPP_INFO(this->get_logger(), "Calibration loaded successfully");
            
            // Initialize camera capture
            RCLCPP_INFO(this->get_logger(), "Initializing camera capture...");
            std::vector<int> camera_indices = {0, 1, 2, 3};  // Default 4 cameras
            capture_ = std::make_shared<CameraCapture>(camera_indices, 1216, 1216, camera_fps);
            ...
            depth_config.matching_width = matching_width_;
            depth_config.matching_height = matching_height_;
...
private:
    ...
    void timer_callback()
    {
        try {
            ...
            cv::Size matching_size(matching_width_, matching_height_);
            cv::Size stitch_size(672, 672);
            if (!capture_->captureImages(images_matching, images_stitch, matching_size, stitch_size)) {
                RCLCPP_WARN(this->get_logger(), "Failed to capture images");
                return;
            }
            
            // 2. Update depth estimation
            cv::Mat rgb_panorama, depth_panorama;
            depth_estimator_->update(images_matching, rgb_panorama, depth_panorama);
            
            // 3. Get depth map
            cv::Mat depth_map = depth_estimator_->getDepthMap();
            
            // 4. Publish RGB panorama
